...
{ 
  ...
  Serial.begin(115200);
  Serial.println("AHR Robot Y MOTOR TEST v1.01");
...
{ 
  ...
  if ((timer_value-timer_old)>=1000)  // 1Khz loop
  {
    ...
    if (loop_counter == 2000)
      {
      Serial.println("Moving the robot to +15cm in Y");
      com_pos_y = ROBOT_MIN_Y + 150;
      setSpeedS(MAX_SPEED_X/8,MAX_SPEED_Y/8);
      setPosition(com_pos_x,com_pos_y);
      }
    if (loop_counter == 4000)
      {
      Serial.println("Moving the robot to back position");
      com_pos_y = ROBOT_MIN_Y;
      setSpeedS(MAX_SPEED_X/4,MAX_SPEED_Y/4);
      setPosition(com_pos_x,com_pos_y); 
      }
    if (loop_counter == 6000)
      ...
      Serial.println("Moving the robot to +15cm in Y");
      com_pos_y = ROBOT_MIN_Y + 150;
      setSpeedS(MAX_SPEED_X/2,MAX_SPEED_Y/2);
      setPosition(com_pos_x,com_pos_y); 
      }
    if (loop_counter == 8000)
      {
      Serial.println("Moving the robot back position");
    ...
    if (loop_counter == 12000)
      {
      Serial.println("Quick movements");
      com_pos_y = ROBOT_MIN_Y + 150;
      setSpeedS(MAX_SPEED_X,MAX_SPEED_Y);
      setPosition(com_pos_x,com_pos_y); 
      }
    if (loop_counter == 12100)
      {
      Serial.println("Quick movements");
      com_pos_y = ROBOT_MIN_Y + 50;
      setSpeedS(MAX_SPEED_X,MAX_SPEED_Y);
      setPosition(com_pos_x,com_pos_y); 
      }
    if (loop_counter == 12200)
      {
      Serial.println("Quick movements");
      com_pos_y = ROBOT_MIN_Y + 150;
      setSpeedS(MAX_SPEED_X,MAX_SPEED_Y);
